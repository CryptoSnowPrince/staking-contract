...
contract MerkleDistributor is OwnableUpgradeable {
    ...
    function withdrawRewards(address rewardCurrency, uint256 amount) external onlyOwner {
        IERC20Upgradeable(rewardCurrency).safeTransfer(msg.sender, amount);
    ...
    function claimMulti(Claim[] memory claims) external notLocked {
        ...
        for (uint256 i = 0; i < claimCount; i++) {
            ...
            if (
                ...
                IERC20Upgradeable(currentRewardToken).safeTransfer(_claim.account, batchedAmount);
                batchedAmount = 0;
            }
        }
    }
    ...
    function verifyClaim(Claim memory _claim) public view returns (bool valid) {
        ...
        return MerkleProofUpgradeable.verify(_claim.merkleProof, merkleWindows[_claim.windowIndex].merkleRoot, leaf);
    }
    ...
    function _setWindow(
        ...
        window.rewardToken = IERC20Upgradeable(rewardToken);
