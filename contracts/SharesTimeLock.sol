...
contract SharesTimeLock is OwnableUpgradeable {
  ...
  function depositByMonths(uint256 amount, uint256 months, address receiver) external {
    ...
    require(
      ...
      "Only whitelisted address can deposit to another address"
    );
  ...
  */
  ...
  function getStakingData(address account) external view returns (StakingData memory data) {
    data.totalStaked = IERC20Upgradeable(depositToken).balanceOf(address(this));
    ...
    data.accountDepositTokenBalance = IERC20Upgradeable(depositToken).balanceOf(account);
    data.accountDepositTokenAllowance = IERC20Upgradeable(depositToken).allowance(account, address(this));
