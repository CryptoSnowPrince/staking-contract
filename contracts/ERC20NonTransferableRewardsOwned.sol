...
contract ERC20NonTransferableRewardsOwned is ERC20NonTransferableRewards, OwnableUpgradeable, AccessControlUpgradeable {
  ...
  function claimFor(address account, bytes32[] memory proof) public {
    ...
    require(MerkleProofUpgradeable.verify(proof, participationMerkleRoot, leaf), "claimFor: Invalid merkle proof");
  ...
  function redistribute(address[] calldata accounts, bytes32[][] calldata proofs) external {
    ...
    for(uint256 i = 0; i < accounts.length; i ++) {
      ...
      if(!MerkleProofUpgradeable.verify(proofs[i], root, leaf)) {
